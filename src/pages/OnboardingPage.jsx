...
const PLANS = {
  beauty: {
    name: "Beauty & Wellness", icon: "\uD83D\uDC87", setup: 250, monthly: 250, leads: 50, perLead: 3,
    features: ["Up to 50 leads/mo included", "$3/lead after 50", "Custom booking AI script", "24/7 call capture", "Email lead notifications", "Analytics dashboard"],
  },
  home_services: {
    name: "Home Services", icon: "\uD83D\uDD27", setup: 500, monthly: 350, leads: 75, perLead: 5, popular: true,
...
export default function OnboardingPage() {
  ...
  const handleStripeCheckout = async () => {
    ...
    try {
      ...
      const res = await fetch(`${BACKEND_URL}/api/v1/stripe/create-checkout`, {
        method: "POST",
      ...
      <div className="relative z-10 w-full max-w-2xl px-6 py-12">
        ...
        <div className="rounded-2xl p-8" style={{ background: "rgba(15,23,42,0.8)", border: "1px solid rgba(6,182,212,0.15)", backdropFilter: "blur(20px)" }}>
          ...
          {step === 1 && (<div>
            ...
            <p className="text-sm mb-8" style={{ color: "#64748b" }}>Let's get your AI lead recovery system set up. First, tell us about your business.</p>
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-medium mb-1.5" style={{ color: "#94a3b8" }}>Your Name</label>
                  <input value={form.contact_name} onChange={e => updateForm("contact_name", e.target.value)} placeholder="John Smith" className={inputStyle} style={inputBg} />
                </div>
                <div>
                  <label className="block text-xs font-medium mb-1.5" style={{ color: "#94a3b8" }}>Business Name</label>
                  <input value={form.business_name} onChange={e => updateForm("business_name", e.target.value)} placeholder="e.g. Premier Plumbing Co" className={inputStyle} style={inputBg} />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-medium mb-1.5" style={{ color: "#94a3b8" }}>Location</label>
                  <input value={form.location} onChange={e => updateForm("location", e.target.value)} placeholder="Miami, FL" className={inputStyle} style={inputBg} />
                </div>
                <div>
                  <label className="block text-xs font-medium mb-1.5" style={{ color: "#94a3b8" }}>Website (optional)</label>
                  <input value={form.website} onChange={e => updateForm("website", e.target.value)} placeholder="www.yourbusiness.com" className={inputStyle} style={inputBg} />
                </div>
              </div>
              <div>
          ...
          {step === 2 && (<div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input value={form.phone} onChange={e => updateForm("phone", e.target.value)} placeholder="(561) 555-0000" className={inputStyle} style={inputBg} />
                <p className="text-xs mt-1" style={{ color: "#475569" }}>We'll set up call forwarding from this number to your AI agent</p>
              </div>
              <div>
                <label className="block text-xs font-medium mb-1.5" style={{ color: "#94a3b8" }}>Lead Notification Email</label>
                <input value={form.notification_email} onChange={e => updateForm("notification_email", e.target.value)} placeholder="leads@yourbusiness.com" className={inputStyle} style={inputBg} />
                <p className="text-xs mt-1" style={{ color: "#475569" }}>Where we send instant lead alerts when AI captures a call</p>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-medium mb-1.5" style={{ color: "#94a3b8" }}>Average Job Value ($)</label>
                  <input value={form.avg_job_value} onChange={e => updateForm("avg_job_value", e.target.value)} placeholder="e.g. 2500" type="number" className={inputStyle} style={inputBg} />
                </div>
                <div>
                  <label className="block text-xs font-medium mb-1.5" style={{ color: "#94a3b8" }}>Services You Offer</label>
                  <input value={form.services_offered} onChange={e => updateForm("services_offered", e.target.value)} placeholder="e.g. Repairs, Installs" className={inputStyle} style={inputBg} />
                </div>
              </div>
              <div className="p-4 rounded-lg" style={{ background: "rgba(6,182,212,0.08)", border: "1px solid rgba(6,182,212,0.15)" }}>
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium" style={{ color: "#e2e8f0" }}>How call forwarding works</p>
                    <p className="text-xs mt-1" style={{ color: "#94a3b8" }}>We'll assign you a dedicated Conduit AI phone number. You set up call forwarding on your business line so missed calls go to your AI agent. We'll send you step-by-step instructions for your carrier within 24-48 hours.</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          {step === 3 && (<div>
            ...
            <p className="text-sm mb-6" style={{ color: "#64748b" }}>Based on your industry, here's the best plan. All plans include a <span style={{ color: "#06b6d4" }}>14-day free trial</span> on the monthly fee.</p>
            <div className="space-y-4">
              {Object.entries(PLANS).map(([key, p]) => {
                const isSelected = detectedPlan === key
                return (
                  <div key={key} className="rounded-xl p-5 transition-all cursor-pointer" onClick={() => {
                    const ind = industries.find(i => i.plan === key)
                    if (ind && !isSelected) updateForm("industry", ind.id)
                  }} style={{
                    background: isSelected ? "rgba(6,182,212,0.1)" : "rgba(30,41,59,0.3)",
                    border: `2px solid ${isSelected ? "rgba(6,182,212,0.5)" : "rgba(100,116,139,0.15)"}`,
                  }}>
                    <div className="flex items-start justify-between">
                      <div>
                        ...
                        <div className="flex items-baseline gap-1 mb-3">
                          <span className="text-3xl font-bold" style={{ color: "#06b6d4" }}>${p.setup}</span>
                          <span className="text-sm" style={{ color: "#64748b" }}>setup</span>
                          <span className="text-sm mx-2" style={{ color: "#475569" }}>+</span>
                      ...
                      <div className="w-6 h-6 rounded-full flex items-center justify-center shrink-0 mt-1" style={{
                        background: isSelected ? "#06b6d4" : "transparent",
                        border: `2px solid ${isSelected ? "#06b6d4" : "rgba(100,116,139,0.3)"}`,
                      }}>{isSelected && <CheckIcon size={14} />}</div>
                    </div>
                  </div>
                )
              })}
            ...
            <div className="mt-4 p-3 rounded-lg text-center" style={{ background: "rgba(16,185,129,0.08)", border: "1px solid rgba(16,185,129,0.15)" }}>
              <p className="text-xs" style={{ color: "#10b981" }}>🛡️ <strong>14-day free trial</strong> on your monthly plan. Pay the setup fee today, your first monthly bill starts in 14 days. Cancel anytime.</p>
            </div>
          </div>)}

          {/* STEP 4: Review & Pay */}
          {step === 4 && (<div>
            <h2 className="text-2xl font-bold mb-2" style={{ color: "#e2e8f0" }}>Review & pay</h2>
            <p className="text-sm mb-6" style={{ color: "#64748b" }}>Confirm your details and complete payment to get started.</p>
            <div className="rounded-xl p-5 mb-4" style={{ background: "rgba(30,41,59,0.5)", border: "1px solid rgba(100,116,139,0.15)" }}>
              <h3 className="text-sm font-semibold mb-3" style={{ color: "#e2e8f0" }}>Order Summary</h3>
              <div className="space-y-2 text-sm" style={{ color: "#94a3b8" }}>
                ...
                <div className="flex justify-between font-semibold"><span>Setup Fee (today)</span><span style={{ color: "#e2e8f0" }}>${plan?.setup}</span></div>
                <div className="flex justify-between"><span>Monthly (starts in 14 days)</span><span style={{ color: "#e2e8f0" }}>${plan?.monthly}/mo</span></div>
              </div>
            </div>
            <button onClick={handleStripeCheckout} disabled={loading}
              className="w-full py-4 rounded-xl text-sm font-bold text-white transition-all disabled:opacity-70"
              style={{ background: "linear-gradient(135deg, #06b6d4, #3b82f6)" }}>
              {loading ? "Redirecting to payment..." : `Pay $${plan?.setup} Setup Fee & Start Free Trial`}
            </button>
            <p className="text-center text-xs mt-3" style={{ color: "#475569" }}>🔒 Secure payment powered by Stripe. Monthly billing starts after your 14-day trial.</p>
          </div>)}
          ...
          {step === 5 && (<div className="text-center py-6">
            ...
            <h2 className="text-2xl font-bold mb-2" style={{ color: "#e2e8f0" }}>Welcome to Conduit AI! 🎉</h2>
            <p className="text-sm mb-6 max-w-md mx-auto" style={{ color: "#64748b" }}>Your AI lead recovery agent is being set up. Here's what happens next:</p>
            <div className="text-left space-y-3 mb-8">
              {[
                { time: "Now", text: "Your dashboard is live — explore it while we set up your AI agent", icon: "📊" },
                { time: "Within 24 hrs", text: "We'll configure your custom AI voice agent for your industry", icon: "🤖" },
                { time: "Within 24 hrs", text: "You'll receive your dedicated Conduit AI phone number via email", icon: "📞" },
                { time: "Within 24 hrs", text: "We'll send step-by-step call forwarding instructions for your carrier", icon: "📋" },
                ...
                <div key={i} className="flex gap-3 p-3 rounded-lg" style={{ background: "rgba(30,41,59,0.3)" }}>
                  ...
                  <div>
                    <span className="text-xs font-semibold" style={{ color: "#06b6d4" }}>{item.time}</span>
                    <p className="text-sm" style={{ color: "#e2e8f0" }}>{item.text}</p>
                  </div>
                </div>
              ))}
            </div>
            <button onClick={() => navigate("/dashboard")} className="px-8 py-3 rounded-xl text-sm font-bold text-white" style={{ background: "linear-gradient(135deg, #06b6d4, #3b82f6)" }}>
              Go to My Dashboard
            </button>
          </div>)}
          ...
          {step < 5 && step !== 4 && (
            <div className="flex justify-between mt-8">
              {step > 1 ? (
                <button onClick={() => setStep(s => s - 1)} className="px-5 py-2.5 rounded-lg text-sm font-medium" style={{ color: "#94a3b8", border: "1px solid rgba(100,116,139,0.3)" }}>Back</button>
              ) : <div />}
              <button onClick={() => setStep(s => s + 1)} className="px-6 py-2.5 rounded-lg text-sm font-semibold text-white" style={{ background: "linear-gradient(135deg, #06b6d4, #3b82f6)" }}>
                Continue
              </button>
            </div>
          )}
        ...
        <p className="text-center text-xs mt-4" style={{ color: "#475569" }}>
          Questions? Call us at <a href="tel:+15614464520" style={{ color: "#06b6d4" }}>(561) 446-4520</a>
        </p>
      </div>
    </div>
  )
