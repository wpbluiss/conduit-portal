...
const CheckCircle = () => <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#10b981" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>
...
export default function OnboardingBanner() {
  ...
  useEffect(() => {
    async function fetchClient() {
      if (!user) return
      const { data } = await supabase.from('clients').select('*').eq('user_id', user.id).limit(1)
      if (data?.[0]) {
        setClient(data[0])
        setForm({
  ...
  const progress = Math.round((completedCount / allFields.length) * 100)
  ...
  const handleSave = async () => {
    setSaving(true)
    const updates = { ...form }
    const allReqFilled = requiredFields.every(f => { const v = updates[f.key]; return v && v.trim() !== '' && v.trim().toLowerCase() !== client.industry })
    if (allReqFilled) updates.onboarding_complete = true
    const { error } = await supabase.from('clients').update(updates).eq('id', client.id)
    if (!error) { setClient(prev => ({ ...prev, ...updates })); setSaved(true); setEditing(false); setTimeout(() => setSaved(false), 3000) }
    setSaving(false)
  }

  const iS = { width: '100%', padding: '10px 14px', borderRadius: '8px', fontSize: '13px', outline: 'none', background: 'rgba(15,23,42,0.8)', border: '1px solid rgba(100,116,139,0.3)', color: '#e2e8f0' }
  const lS = { display: 'block', fontSize: '12px', fontWeight: 600, color: '#94a3b8', marginBottom: '6px' }
        ...
        {expanded && editing && (
          <div style={{ marginTop: '20px' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '10px', marginTop: '20px' }}>
              <button onClick={() => { setEditing(false); setExpanded(false) }} style={{ padding: '10px 20px', borderRadius: '8px', fontSize: '13px', fontWeight: 600, background: 'transparent', color: '#94a3b8', border: '1px solid rgba(100,116,139,0.3)', cursor: 'pointer' }}>Cancel</button>
              <button onClick={handleSave} disabled={saving} style={{ padding: '10px 24px', borderRadius: '8px', fontSize: '13px', fontWeight: 700, background: 'linear-gradient(135deg, #f59e0b, #f97316)', color: 'white', border: 'none', cursor: 'pointer', opacity: saving ? 0.7 : 1 }}>{saving ? 'Saving...' : 'Save & Continue Setup'}</button>
            </div>
          </div>
        )}
      </div>
